...
threespinestickleback.mixed.model.mass <-lmer(weight_g ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.threespinestickleback)
ggplot(fish.data.clean2.threespinestickleback, aes(x=fishingTemperature_C, y=n)) +
  ...
  xlab("Temperature (degrees C)") +
  theme(plot.title = element_text(hjust = 0.5))
ggplot(fish.data.clean2.threespinestickleback, aes(x=fishingTemperature_C, y=weight_g)) +
  geom_point() +
  geom_smooth()+
  theme_classic() +
...
```{r}
slimysculpin.result <- lm(slimy.sculpin~round.goby, data=fish.data.wider[which(fish.data.wider$slimy.sculpin>0 & fish.data.wider$round.goby>0),])
...
#to do list
fix mixed model graphs to show the calculated line not a smooth line
make a table of each model and its significance or not significance
check assumptions on the models we ran
make presentation
finish presentation notes
