...
fish.data.raw <- read.csv("Dataset/TrawlCatch_SpringPreyfishBottomTrawl (1).csv")
head(fish.data.raw)
...
fish.data <- fish.data.raw %>% 
  #remove any data points where the temperature, latitude, or longitude is not collected because they will not be useful for the questions we are asking unless we have those data
  #remove unidentified or misc. fishes
  filter(!is.na(fishingTemperature_C), !is.na(latitude), !is.na(longitude), commonName!= "No fish caught", commonName!= "Miscellaneous or unidentified species", commonName!=  "Unidentified coregonid", commonName!= "Unidentified minnows",  commonName!= "Uninidentified redhorse")
...
# Proportion of the total catch from the first occurrence of the Round Goby in 1997 
# fish.data.clean
fish.data.clean %>% 
...
```
```{r}
#johnny darter mixed model
johnnydarter.mixed.model.abundance <- lmer(n ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.johnnydarter)
johnnydarter.mixed.model.mass <-lmer(average_weight ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.johnnydarter)
...
```
```{r}
#lake trout mixed model
laketrout.mixed.model.abundance <- lmer(n ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.laketrout)
laketrout.mixed.model.mass <-lmer(average_weight ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.laketrout)
...
```
```{r}
#slimy sculpin mixed model
slimysculpin.mixed.model.abundance <- lmer(n ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.slimysculpin)
slimysculpin.mixed.model.mass <-lmer(average_weight ~ fishingTemperature_C + (1|.cluster), REML = F, data=fish.data.clean2.slimysculpin)
...
```
```{r}
#Threespine stickleback mixed model
...
```{r}
# threespine stickleback linear model
...
```{r}
# deepwater sculpin linear model
